<template>
  <div
    ref="symbolRef"
    class="w-full grid grid-cols-10 text-[#777E90] text-base h-[76px]"
  ...
  >
    <div class="flex items-center justify-start col-span-4">
      <div class="w-[178px] text-center flex items-center">
        <img
          src="@/static/brain/rating-star.png"
          alt=""
          class="w-6 h-6"
          v-if="isFavorite"
        ...
        <div class="ml-4 flex items-center gap-2">
          <img src="@/static/brain/vip.png" alt="" class="w-8 h-8" />
          <div>
            <div class="flex items-center gap-2">
              <div class="text-lg font-bold text-[#666666]">
                {{ props.item.symbolShowName }}
              </div>
            </div>
            <div class="text-xs text-[#B1B5C3] mt-2 text-left">
              {{ props.item.symbolShowName }}
            </div>
          </div>
        </div>
    ...
    <div class="col-span-6 grid grid-cols-3">
      <div class="flex items-center justify-start text-base">
        {{ `$ ${props.item.price || 0}` }}
      </div>
      <div class="flex items-center justify-center gap-1">
        <div :class="getTextColor(props.item.rate)">
          {{ `${props.item.rate <= 0 ? "" : "+"}${props.item.rate || 0}%` }}
        </div>
        <img
          src="@/static/brain/check-small.png"
          alt=""
          class="w-[14px] h-[14px]"
          v-if="props.item.rate > 0"
        />
        <img
          src="@/static/brain/check-small.png"
          alt=""
...
import {
  reqAddUserSelectMarket,
  reqDeleteUserSelectMarket,
  reqGetMarketHistoryData,
  reqGetMarketNewData,
} from "../api/apis";
...
const nowMarketDetail = computed(
  () => marketStore.allMarketObjects[props.item.symbol],
);
const isFavorite = computed(() =>
  tradeStore.userSelectMarketList.find((i) => i.symbol === props.item.symbol),
);
const history = ref([]);
...
watch(nowMarketDetail, (newVal, oldVal) => {
  if (newVal) {
    ...
    props.item.rate = new BigNumber(props.item.price)
      .minus(props.item.lastClosePrice)
      .dividedBy(props.item.lastClosePrice)
      .multipliedBy(100)
      .toFixed(2);
