<template>
    <section class="skye-home-embed">
        ...
        <!-- <section class="skye-service-buttons">
            <button
                v-for="item in serviceButtons"
                :key="item.labelKey"
                class="skye-service-button"
                type="button"
            >
                {{ t(item.labelKey) }}
            </button>
            ...
            <div class="skye-expert-stage" :style="expertStageStyle">
                <div class="skye-expert-card">
                    ...
                    <p class="skye-expert-copy">
                        {{ t("sk_expert_p1") }} {{ t("sk_expert_p2") }}
                        {{ t("sk_expert_p3") }} {{ t("sk_expert_p4") }}
                        {{ t("sk_expert_p5") }} {{ t("sk_expert_p6") }}
                    </p>
        ...
        <section class="skye-split-section skye-how-section">
            <div class="skye-split-content">
                ...
                <p>
                    <strong>{{ t("sk_brand") }}</strong> {{ t("sk_how_copy") }}
                </p>
        ...
        <section class="skye-clients-hero">
            <div class="skye-clients-hero__inner">
                <h2 class="skye-clients-hero__title">
                    ...
                    <span class="skye-clients-hero__title-text">{{
                        t("sk_clients")
                    }}</span>
                </h2>
                <p class="skye-clients-hero__desc">
                    {{ t("sk_clients_copy") }}
                </p>
                <!-- <button class="skye-clients-hero__button" type="button">
                    {{ t("sk_clients_btn") }}
                </button> -->
            </div>
        </section>
        ...
        <section class="skye-services-carousel">
            <div class="skye-service-viewport">
                <div
                    ...
                    :style="{
                        transform: `translate3d(-${serviceTrackIndex * 100}%, 0, 0)`,
                    }"
                    @transitionend="handleServiceTransitionEnd"
                >
                    <div
                        v-for="(page, pageIndex) in serviceCarouselPages"
                        :key="getServicePageKey(page, pageIndex)"
                        class="skye-service-page"
                    >
                        <article
                            v-for="service in page"
                            :key="service.titleKey"
                            class="skye-service-card"
                        >
                            <div class="skye-service-card__image-wrap">
                ...
                <button
                    type="button"
                    class="skye-service-arrow skye-service-arrow--prev"
                    :aria-label="t('sk_prev_services')"
                    @click="handleServicePrev"
                >
                    ‹
                </button>
                <button
                    type="button"
                    class="skye-service-arrow skye-service-arrow--next"
                    :aria-label="t('sk_next_services')"
                    @click="handleServiceNext"
                >
                    ›
                </button>
            </div>
        ...
        <!-- <section class="skye-reviews-section">
            ...
            >
                <div
                    ...
                    :style="{
                        transform: `translate3d(-${reviewTrackIndex * 100}%, 0, 0)`,
                    }"
                    @transitionend="handleReviewTransitionEnd"
                >
                    <article
                        v-for="(review, reviewIndex) in reviewCarouselItems"
                        :key="getReviewItemKey(review, reviewIndex)"
                        class="skye-review-slide"
                    >
                        <div class="skye-review-card">
                            <div class="skye-review-card__head">
                                <div
                                    class="skye-review-avatar"
                                ...
                                <div class="skye-review-card__meta">
                                    <div class="skye-review-card__name">
                                        {{ review.name }}
                                    </div>
                                    <div class="skye-review-card__time">
                                        {{ t(review.timeKey) }}
                                    </div>
                                </div>
                                <div
                                    class="skye-review-source"
                                    :class="review.sourceClass"
                                >
                                    <img
                                        v-if="review.sourceIcon"
                            ...
                            <div class="skye-review-stars">
                                <span>★★★★★</span
                                ><span class="skye-review-verified">●</span>
                            </div>
                            <p class="skye-review-text">
                                {{ t(review.textKey) }}
                            </p>
                            <button
                                v-if="review.readMore"
                                type="button"
                                class="skye-review-read-more"
                            >
                                {{ t("sk_read_more") }}
                            </button>
                        </div>
            ...
            <div class="skye-review-progress">
                <div class="skye-review-progress__bar">
                    <span
                        class="skye-review-progress__thumb"
                        :style="{ left: reviewProgressLeft }"
                    ></span>
                </div>
            </div>
            <div class="skye-review-score">
                <strong>{{ t("sk_trustindex") }}</strong>
                {{ t("sk_score_text") }} <strong>5.0</strong>
        ...
        <section class="skye-insights-section">
            ...
            <div class="skye-insights-inner">
                <h2 class="skye-insights-title">
                    <span>{{ t("sk_brand") }}</span
                    ><span>{{ t("sk_insights") }}</span>
                </h2>
                <div class="skye-insights-list">
                    <article
                        v-for="post in insightPosts"
                        :key="post.titleKey"
                        class="skye-insight-card"
                    >
                        <div class="skye-insight-card__image-wrap">
                        ...
                        <div class="skye-insight-card__body">
                            ...
                            <div class="skye-insight-card__meta">
                                <span
                                    >{{ t("sk_published") }}:
                                    {{ t(post.dateKey) }}</span
                                ><span
                                    >{{ t("sk_views") }}: {{ post.views }}</span
                                >
                            </div>
                            <p
                                v-if="post.excerptKey"
                                class="skye-insight-card__excerpt"
                            >
                                {{ t(post.excerptKey) }}
                            </p>
                            <!-- <button
                                type="button"
                                class="skye-insight-card__button"
                            >
                                <span>➜</span> {{ t("sk_continue") }}
                            </button> -->
                        </div>
                    </article>
                </div>
        ...
        <!-- <section class="skye-talk-section">
            <div class="skye-talk-art" :aria-label="t('sk_talk_label')">
                ...
                <span class="skye-talk-art__text skye-talk-art__text--talk">{{
                    t("sk_talk")
                }}</span>
            </div>
        ...
        <footer class="skye-footer">
            ...
            >
                <span
                    v-for="item in socialItems"
                    :key="`footer-${item.key}`"
                    class="skye-footer__social-icon"
                >
                    <img :src="item.icon" :alt="item.label" />
                </span>
            </div> -->
            ...
            <div class="skye-footer-badge" :aria-label="t('sk_badge_label')">
                <div
                    class="skye-footer-badge__ribbon skye-footer-badge__ribbon--left"
                ></div>
                <div
                    class="skye-footer-badge__ribbon skye-footer-badge__ribbon--right"
                ></div>
                <div class="skye-footer-badge__circle">
                    <div class="skye-footer-badge__stars">★ ★ ★ ★ ★</div>
                    <div class="skye-footer-badge__were">
                        {{ t("sk_were") }}
                    </div>
                    <div class="skye-footer-badge__banner">
                        {{ t("sk_highly") }}<br />{{ t("sk_recommended") }}
                    </div>
                    <div class="skye-footer-badge__local">
                        {{ t("sk_by_locals") }}<br />{{ t("sk_on_alignable") }}
                    </div>
                </div>
            </div>
...
const logo = new URL(
    "@/static/images/skye-home/SO-logo-black.svg",
    import.meta.url,
).href;
const whiteLogo = new URL(
    "@/static/images/skye-home/SO-logo-outlines-white.svg",
    import.meta.url,
).href;
const heroImage = new URL(
    "@/static/images/skye-home/tqaguwpax5k.jpg",
    import.meta.url,
).href;
const expertImage = new URL(
    "@/static/images/skye-home/t6fdn60bmwy.jpg",
    import.meta.url,
).href;
const insightImageOne = new URL(
    "@/static/images/skye-home/featured-34753.jpg",
    import.meta.url,
).href;
const insightImageTwo = new URL(
    "@/static/images/skye-home/featured-34754.jpg",
    import.meta.url,
).href;
const insightImageThree = new URL(
    "@/static/images/skye-home/featured-34755.jpg",
    import.meta.url,
).href;
const webDesignIcon = new URL(
    "@/static/images/skye-home/sos-web-design-and-dev-final.svg",
    import.meta.url,
).href;
const brandingIcon = new URL(
    "@/static/images/skye-home/sos-branding-and-design-final.svg",
    import.meta.url,
).href;
const hostingIcon = new URL(
    "@/static/images/skye-home/sos-web-hosting-final.svg",
    import.meta.url,
).href;
const maintenanceIcon = new URL(
    "@/static/images/skye-home/sos-web-maintenance-final.svg",
    import.meta.url,
).href;
const seoIcon = new URL(
    "@/static/images/skye-home/sos-seo-final.svg",
    import.meta.url,
).href;
const socialIcon = new URL(
    "@/static/images/skye-home/sos-social-media-final.svg",
    import.meta.url,
).href;
const lightLogo = new URL(
    "@/static/images/skye-home/logo-lightbackx86.png",
    import.meta.url,
).href;
const stvdLogo = new URL(
    "@/static/images/skye-home/STVDlogoblueonwhiteR-2-1.png",
    import.meta.url,
).href;
const puppiesLogo = new URL(
    "@/static/images/skye-home/HappyFarmPuppies-NewLogo.png",
    import.meta.url,
).href;
const keadyLogo = new URL(
    "@/static/images/skye-home/KeadyLawFirm-Logo-Final.png",
    import.meta.url,
).href;
const capitalLogo = new URL(
    "@/static/images/skye-home/horizontal-logo-72h.png",
    import.meta.url,
).href;
const parishLogo = new URL(
    "@/static/images/skye-home/logo-512x512-1.png",
    import.meta.url,
).href;
const reviewAvatar1 = new URL(
    "@/static/images/skye-home/unnamed.png",
    import.meta.url,
).href;
const reviewAvatar2 = new URL(
    "@/static/images/skye-home/unnamed(1).png",
    import.meta.url,
).href;
const reviewAvatar3 = new URL(
    "@/static/images/skye-home/unnamed(2).png",
    import.meta.url,
).href;
const reviewAvatar4 = new URL(
    "@/static/images/skye-home/unnamed(3).png",
    import.meta.url,
).href;
const reviewAvatar5 = new URL(
    "@/static/images/skye-home/unnamed(4).png",
    import.meta.url,
).href;
const reviewAvatar6 = new URL(
    "@/static/images/skye-home/unnamed(5).png",
    import.meta.url,
).href;
const reviewAvatar7 = new URL(
    "@/static/images/skye-home/unnamed(6).png",
    import.meta.url,
).href;
const reviewAvatar8 = new URL(
    "@/static/images/skye-home/unnamed(7).png",
    import.meta.url,
).href;
const reviewAvatar9 = new URL(
    "@/static/images/skye-home/unnamed(8).png",
    import.meta.url,
).href;
const reviewAvatar10 = new URL(
    "@/static/images/skye-home/noprofile-10.svg",
    import.meta.url,
).href;
const reviewGoogleIcon = new URL(
    "@/static/images/skye-home/review-google.svg",
    import.meta.url,
).href;
const reviewFacebookIcon = new URL(
    "@/static/images/skye-home/review-facebook.svg",
    import.meta.url,
).href;

const socialFacebookIcon = new URL(
    "@/static/images/skye-home/social-facebook.svg",
    import.meta.url,
).href;
const socialXIcon = new URL(
    "@/static/images/skye-home/social-x.svg",
    import.meta.url,
).href;
const socialInstagramIcon = new URL(
    "@/static/images/skye-home/social-instagram.svg",
    import.meta.url,
).href;
const socialPinterestIcon = new URL(
    "@/static/images/skye-home/social-pinterest.svg",
    import.meta.url,
).href;
const socialTiktokIcon = new URL(
    "@/static/images/skye-home/social-tiktok.svg",
    import.meta.url,
).href;
const socialYoutubeIcon = new URL(
    "@/static/images/skye-home/social-youtube.svg",
    import.meta.url,
).href;
const socialLinkedinIcon = new URL(
    "@/static/images/skye-home/social-linkedin.svg",
    import.meta.url,
).href;
const socialDiscordIcon = new URL(
    "@/static/images/skye-home/social-discord.svg",
    import.meta.url,
).href;
const socialPhoneIcon = new URL(
    "@/static/images/skye-home/social-phone.svg",
    import.meta.url,
).href;
...
const reviews = [
    {
        ...
        timeKey: "sk_time_9m",
        source: "Google",
        sourceClass: "is-google",
        sourceIcon: reviewGoogleIcon,
        avatar: reviewAvatar1,
        textKey: "sk_review_1",
    },
    {
        name: "CARLOS vegerano",
        timeKey: "sk_time_9m",
        source: "Google",
        sourceClass: "is-google",
        sourceIcon: reviewGoogleIcon,
    ...
    },
    {
        ...
        timeKey: "sk_time_6y",
        source: "Facebook",
        sourceClass: "is-facebook",
        sourceIcon: reviewFacebookIcon,
    ...
    },
    {
        ...
        textKey: "sk_review_10",
    },
];
const insightPosts = [
    {
        titleKey: "sk_post1_title",
        dateKey: "sk_post1_date",
        views: 5,
        image: insightImageOne,
    },
    {
        titleKey: "sk_post2_title",
        dateKey: "sk_post2_date",
        views: 50,
        image: insightImageTwo,
        excerptKey: "sk_post2_excerpt",
    },
    {
        titleKey: "sk_post3_title",
        dateKey: "sk_post3_date",
        views: 172,
        image: insightImageThree,
        excerptKey: "sk_post3_excerpt",
    },
];
...
const servicePages = computed(() => {
    ...
    for (let i = 0; i < services.length; i += servicePageSize) {
        ...
        if (page.length < servicePageSize && services.length)
            page.push(...services.slice(0, servicePageSize - page.length));
        pages.push(page);
    }
    return pages;
});
...
const getServicePageKey = (page, pageIndex) =>
    `${pageIndex}-${page.map((item) => item.titleKey).join("-")}`;
...
const handleServiceTransitionEnd = (event) => {
    ...
    if (serviceTrackIndex.value === 0)
        return setServiceTrackIndexWithoutAnimation(realPageCount);
    if (serviceTrackIndex.value === realPageCount + 1)
        return setServiceTrackIndexWithoutAnimation(1);
    isServiceSliding.value = false;
};
...
const reviewCarouselItems = computed(() =>
    reviews.length ? [reviews[reviews.length - 1], ...reviews, reviews[0]] : [],
);
const getReviewItemKey = (review, reviewIndex) =>
    `${reviewIndex}-${review.name}-${review.timeKey}`;
const reviewProgressLeft = computed(() =>
    reviews.length <= 1
        ? "0%"
        : `${(activeReviewIndex.value / (reviews.length - 1)) * 88}%`,
);
...
const handleReviewPrev = () => {
    ...
    activeReviewIndex.value =
        (activeReviewIndex.value - 1 + reviews.length) % reviews.length;
...
const handleReviewTransitionEnd = (event) => {
    if (
        ...
        return;
    if (reviewTrackIndex.value === 0)
        return setReviewTrackIndexWithoutAnimation(reviews.length);
    if (reviewTrackIndex.value === reviews.length + 1)
        return setReviewTrackIndexWithoutAnimation(1);
    isReviewSliding.value = false;
};
...
const handleReviewTouchMove = (event) => {
    if (isReviewSliding.value) return;
    reviewTouchCurrentX.value =
        event.touches?.[0]?.clientX || reviewTouchCurrentX.value;
};
...
const handleReviewMouseStart = (event) => {
    if (
        !isReviewMouseDragEnabled() ||
        isReviewSliding.value ||
        event.button !== 0
    )
        return;
    ...
    window.addEventListener("mousemove", handleReviewMouseMove, {
        passive: false,
    });
...
.skye-topic-strip__shape {
    ...
    box-shadow: 0 3px 10px rgba(89, 105, 102, 0.08);
...
.skye-insights-bg {
    ...
    background-image:
        radial-gradient(
            circle at 18px 18px,
            transparent 0 15px,
            rgba(96, 119, 105, 0.35) 16px,
            transparent 17px
        ),
        radial-gradient(
            circle at 78px 72px,
            transparent 0 21px,
            rgba(96, 119, 105, 0.28) 22px,
            transparent 23px
        );
