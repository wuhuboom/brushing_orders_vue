<template>
  <div class="w-full">
    <div class="flex items-center">
      <span v-if="prefix" class="text-base"
        >{{ prefix
        }}<span class="text-sm opacity-50 ml-2.5" v-if="lastPrefix">{{
          lastPrefix
        }}</span></span
      >
      <div class="ml-auto" v-if="showSet">
        <img
          src="@/static/images/selected-check.png"
          alt=""
          class="w-5 h-5 ml-atuo"
          v-if="props.isSet"
          @click="handleChangeIsSet"
        />
        <img
          src="@/static/images/selected-uncheck.png"
          alt=""
          class="w-5 h-5 ml-atuo"
          v-else
          @click="handleChangeIsSet"
    ...
    <div class="flex items-center gap-2 mt-4">
      <div
        class="bg-gray w-[50px] h-[50px] flex items-center justify-center rounded-2.5"
        @click="decrement"
      >
        <el-icon size="24">
          <Minus />
      ...
      <input
        ...
        class="flex-1 bg-gray h-[50px] rounded-2.5 text-center text-base"
        :placeholder="placeholder"
        :disabled="!props.isSet"
      />
      <div
        class="bg-gray w-[50px] h-[50px] flex items-center justify-center rounded-2.5"
        @click="increment"
