<template>
    <el-drawer v-model="dialogVisible" :show-close="false" :modal="true" direction="btt" append-to-body size="auto"
        ...
        <p class="text-[25px] text-black">{{ $t('选择银行卡') }}</p>
        <div class="pt-5 flex flex-col gap-2.5">
            <div class="flex items-start gap-4" @click="handleSelect(item)" v-for="item in list">
                ...
                <div class="flex-1 p-4 rounded-2xl flex items-center bg-blue text-white"
                    ...
                    <el-icon class="text-white ml-auto" size="18" v-if="item.id == modelValue"><Select /></el-icon>
                </div>
            </div>
        </div>
    </el-drawer>
