<template>
  <el-drawer v-model="dialogVisible" :show-close="false" :modal="true" direction="btt" append-to-body size="auto"
    ...
    <div>
      <p class=" opacity-50">{{ $t('确认删除这个{name}吗？', { name: props.type === 'crypto' ? $t('加密钱包') : $t('银行卡') }) }}</p>
      <div class="flex items-center gap-5 mt-8">
        <p class="flex-1 flex items-center justify-center bg-gray rounded-2xl h-[50px]" @click="close">{{ $t('取消') }}
        </p>
