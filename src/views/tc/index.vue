<template>
    <div class="clause-page">
        ...
        <div class="clause-body">
            <section class="clause-hero">
                <div class="clause-hero__main">
                    ...
                    <div class="clause-hero__content">
                        ...
                        <div
                            v-if="tcHeroIntroTitleList.length"
                            class="clause-hero__intro"
                        >
                            <p
                                v-for="(item, index) in tcHeroIntroTitleList"
                                :key="`hero-intro-title-${index}`"
                                v-html="item"
                            ></p>
                        </div>
                    </div>
                ...
                <div class="clause-hero__note">
                    <p
                        v-for="(item, index) in tcHeroIntroNoteList"
                        :key="`hero-intro-note-${index}`"
                        v-html="item"
                    ></p>

                    <p
                        v-for="(item, index) in tcHeroNotice"
                        :key="`hero-notice-${index}`"
                        v-html="item"
                    ></p>
                </div>
            </section>

            <section v-if="tcSections.length" class="clause-toc-card">
                <div class="clause-section-title">
                    {{ $t("tc_table_of_contents") }}
                </div>

                <div class="clause-toc-list">
                    <div
                        v-for="(item, index) in tcSections"
                        :key="`toc-${index}`"
            ...
            <section v-if="tcSections.length" class="clause-accordion">
                <van-collapse
                    v-model="activeNames"
                    :border="false"
                ...
                >
                    <van-collapse-item
                        v-for="(section, index) in tcSections"
                    ...
                    >
                        ...
                        >
                            <div
                                ...
                                class="clause-card__content-item"
                            >
                                <div class="clause-card__content-no">
                                    {{ contentItem.no }}
                                </div>

                                <div
                                    class="clause-card__content-text rich-content"
            ...
            <div class="clause-alert">
                <img src="@/static/images/tgcellwar.png" alt="" />
                <span>
                    {{ $t("tc_update_alert") }}
                </span>
            </div>
...
const tcHeroIntro = computed(() => {
    return formatArray(tm("tc_hero_intro"));
});

const tcHeroIntroTitleList = computed(() => {
    return tcHeroIntro.value.slice(0, 1);
});

const tcHeroIntroNoteList = computed(() => {
    return tcHeroIntro.value.slice(1);
});

const tcHeroNotice = computed(() => {
    return formatArray(tm("tc_hero_notice"));
});

const tcSections = computed(() => {
    const sections = tm("tc_terms_sections");
...
.clause-hero__intro p:last-child {
    margin-bottom: 0;
}

.clause-hero__intro :deep(strong),
.clause-hero__note :deep(strong) {
    color: #fff;
    font-weight: 700;
}
...
.clause-hero__note p:last-child {
    margin-bottom: 0;
}
...
.clause-card__content-item:last-child {
    margin-bottom: 0;
}

.clause-card__content-no {
    min-width: 34px;
    color: #2aa24d;
    font-size: 13px;
    line-height: 24px;
